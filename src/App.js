...
function App() {
  ...
  let time_scale = (mp) => { if( mp === undefined) { return(state["TIME_SCALE"]);} else {setState({...state, "TIME_SCALE" : mp})} }
  let time_offset = (mp) => { if( mp === undefined) { return(state["TIME_OFFSET"]);} else {console.log("setState TimeOffset=" + mp);setState({...state, "TIME_OFFSET" : mp})} }
  ...
  function draw(context, canvas) {
    ...
    let coord = waveforms.map((time, index) => {
  ...
  let mscbFunc = (m,x,y) => { 
    if(m === "DOWN") { setState({...state, "MOUSE" : {...state["MOUSE"], "DOWN" : [x,y,1], "MOVE" : [0,0,0], "UP" : [0,0,0]}}) } 
    if(m === "UP") { setState({...state, "MOUSE" : {...state["MOUSE"], "DOWN" : [0,0,0], "MOVE" : [0,0,0], "UP" : [x,y,1]}}) } 
    if(m === "MOVE" && state["MOUSE"]["DOWN"][2] === 1) { 
      var dn1 = state["MOUSE"]["DOWN"]; var mv1 = state["MOUSE"]["MOVE"]
      if(dn1[2] === 1) { time_offset((x - dn1[0])/2); console.log("Setting time offset " + ((x - dn1[0])/2) + "   actual value=" + time_offset())}
      console.log("---------- Mouse -----------------")
      console.log(state["MOUSE"])
      setState({...state, "MOUSE" : {...state["MOUSE"], "DOWN" : state["MOUSE"]["DOWN"], "MOVE" : [x,y,1], "UP" : state["MOUSE"]["UP"]}}) 
      } 
  } 
  return (
    <div className="App">
